...
if uploaded_files:
    ...
    for file in uploaded_files:
        ...
        candidate_data.append({
            ...
            "Education": ", ".join(candidate_info["Education"]),
        ...
        with st.expander(file.name):
            ...
            st.write(
                f"{candidate_info['Experience']} years"
            )
            st.write("**Education:**")
    ...
    average_score = round(
     candidate_df["Match Score"].mean(),
         2
    )

    top_score = candidate_df["Match Score"].max()
    ...
    for rank, candidate in candidate_df.iterrows():
    ...
    st.info("Please upload candidate resumes.")

selected_data = candidate_df[
    candidate_df["Name"] == selected_candidate
].iloc[0]
...
with col1:
    ...
    st.write(
        "**Experience:**",
        f"{selected_data['Experience']} years"
    )

    st.write(
        "**Education:**",
        selected_data["Education"]
    )
...
with col2:
    ...
    st.metric(
        "Overall Match",
        f"{selected_data['Match Score']}%"
    )

    st.metric(
        "Skill Score",
        f"{selected_data['Skill Score']}%"
    )

    st.metric(
        "Experience Score",
        f"{selected_data['Experience Score']}%"
    )
...
if st.button(
    ...
    st.success(
        f"{selected_candidate} saved successfully!"
    )
...
if saved_candidates:
    ...
    st.info(
        "No recruitment decisions saved yet."
    )

st.subheader("⭐ Shortlisted Candidates")

shortlisted = [
    candidate for candidate in saved_candidates
    if candidate[10] == "Shortlisted"
]
